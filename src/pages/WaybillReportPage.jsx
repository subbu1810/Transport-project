...
function WaybillReportPage() {
  ...
  const [waybills] = useState([
    { billDate: '', gcNum: '', destination: '', invoiceNo: '', articles: '', weight: '', freight: '', ddCharges: '', handling: '', stationary: '', serviceTax: '', total: '', status: '', deliverStatus: '', amountPaid: '' }
  ])

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-3xl font-bold text-gray-800">Waybill Report</h1>

      <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
        <div className="bg-green-50 p-6 rounded-lg border-2 border-green-300">
          <h3 className="font-bold text-gray-800 mb-4">Waybill Report</h3>
              ...
              <tbody>
                {waybills.map((wb, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    <td className="px-3 py-2">{wb.billDate}</td>
                    <td className="px-3 py-2">{wb.gcNum}</td>
                    <td className="px-3 py-2">{wb.destination}</td>
                    <td className="px-3 py-2">{wb.invoiceNo}</td>
                    <td className="px-3 py-2">{wb.articles}</td>
                    <td className="px-3 py-2">{wb.weight}</td>
                    <td className="px-3 py-2">{wb.freight}</td>
                    <td className="px-3 py-2">{wb.ddCharges}</td>
                    <td className="px-3 py-2">{wb.handling}</td>
                    <td className="px-3 py-2">{wb.stationary}</td>
                    <td className="px-3 py-2">{wb.serviceTax}</td>
                    <td className="px-3 py-2">{wb.total}</td>
                    <td className="px-3 py-2">{wb.status}</td>
                    <td className="px-3 py-2">{wb.deliverStatus}</td>
                    <td className="px-3 py-2">{wb.amountPaid}</td>
                  </tr>
                ))}
              </tbody>
            </table>
