...
function ConsolidatedEWayBillView() {
  ...
  const [tripsheets] = useState([
    { consEWayBillNo: '', generatedDate: '', tripsheetNo: '', dispatchDate: '', vehicleNo: '', driverName: '', ownerName: '', advance: '', tripRemarks: '', status: '', askDate: '', askRemarks: '' }
  ])

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-3xl font-bold text-gray-800">Consolidated E-WayBill View</h1>

      <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
        <div className="bg-green-50 p-6 rounded-lg border-2 border-green-300">
          <h3 className="font-bold text-gray-800 mb-4">Consolidated Ewaybill Tripsheet Details</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-yellow-100 border-b-2 border-yellow-300">
                <tr>
                  <th className="px-4 py-2 text-left font-semibold">Cons E-WayBill No</th>
                  <th className="px-4 py-2 text-left font-semibold">Generated Date/Time</th>
                  ...
                  <th className="px-4 py-2 text-left font-semibold">Ack Date</th>
                  <th className="px-4 py-2 text-left font-semibold">Ack Remarks</th>
                </tr>
              </thead>
              <tbody>
                {tripsheets.map((ts, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    <td className="px-4 py-2">{ts.consEWayBillNo}</td>
                    <td className="px-4 py-2">{ts.generatedDate}</td>
                    <td className="px-4 py-2">{ts.tripsheetNo}</td>
                    <td className="px-4 py-2">{ts.dispatchDate}</td>
                    <td className="px-4 py-2">{ts.vehicleNo}</td>
                    <td className="px-4 py-2">{ts.driverName}</td>
                    <td className="px-4 py-2">{ts.ownerName}</td>
                    <td className="px-4 py-2">{ts.advance}</td>
                    <td className="px-4 py-2">{ts.tripRemarks}</td>
                    <td className="px-4 py-2">{ts.status}</td>
                    <td className="px-4 py-2">{ts.askDate}</td>
                    <td className="px-4 py-2">{ts.askRemarks}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export default ConsolidatedEWayBillView
