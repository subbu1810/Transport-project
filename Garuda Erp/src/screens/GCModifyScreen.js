...
export default function GCModifyScreen({ navigation }) {
  const [loading, setLoading] = useState(true);
  const [searching, setSearching] = useState(false);
  const [gcSearchNumber, setGcSearchNumber] = useState('');
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (userStr) setCurrentUser(userObj);
      ...
      if (response.data.success) {
        ...
        let userBranchId = userObj?.branch_id || '';

        let filteredBranches = br || [];
        let filteredConsignors = cons || [];

        if (userRole === 'superadmin') {
          setOriginBranches(filteredBranches);
          setConsignors(filteredConsignors);
        } else {
          const myBranch = filteredBranches.find(b => parseInt(b.id) === parseInt(userBranchId));
          setOriginBranches(myBranch ? [myBranch] : []);
          setFormData(prev => ({ ...prev, from: userBranchId.toString() }));
          setConsignors(filteredConsignors.filter(c => parseInt(c.branch_id) === parseInt(userBranchId)));
        }
  ...
  const searchGC = async () => {
    if (!gcSearchNumber) {
      Alert.alert('Validation Error', 'Please enter a GC Number to search.');
      return;
    }
    
    setSearching(true);
    try {
      const response = await api.get(`/waybills/search/${gcSearchNumber}`);
      if (response.data.success && response.data.data) {
        const wb = response.data.data;

        // Role Validation for Modification
        if (currentUser && currentUser.role !== 'superadmin') {
          if (parseInt(wb.origin_branch_id) !== parseInt(currentUser.branch_id)) {
            Alert.alert('Unauthorized', 'You are not authorized to modify a GC created at another branch.');
            setSearching(false);
            return;
          }
        }
        ...
        setFormData({
          billDate: wb.bill_date ? wb.bill_date.split('T')[0] : new Date().toISOString().split('T')[0],
          from: wb.origin_branch_id?.toString() || '',
          to: wb.destination_id?.toString() || '',
          consignor: wb.consignor_id?.toString() || '',
          consignee: wb.consignee_id?.toString() || '',
          articleDesc: wb.article_desc || '',
          totalArticles: wb.total_articles?.toString() || '0',
          freight: wb.freight_amount?.toString() || '0',
          dd: wb.dd_charges?.toString() || '0',
          handling: wb.handling_charges?.toString() || '0',
          stationary: wb.stationary_charges?.toString() || '0',
          totalAmount: wb.total_amount?.toString() || '0',
          invoiceNo: wb.invoice_no || '',
          declared: wb.declared_value?.toString() || '0',
          eWayBillNo: wb.eway_bill_no || '',
          taxPayableBy: wb.tax_payable_by || '',
          accountType: wb.account_type?.toLowerCase() || '',
          cashReceived: wb.cash_received == 1 || wb.cash_received === true,
          gstPercent: wb.gst_percent?.toString() || '0',
          gstAmount: wb.gst_amount?.toString() || '0',
          grandTotal: wb.grand_total?.toString() || '0',
          remarks: wb.remarks || '',
        });

        if (wb.articles && wb.articles.length > 0) {
          setArticles(wb.articles.map((art, index) => ({
            id: index,
            type: art.article_type || '',
            noOfArticles: art.no_of_articles?.toString() || '0',
            rate: parseFloat(art.rate) || 0,
            total: parseFloat(art.total) || 0,
            ddRate: parseFloat(art.dd_rate) || 0,
            ddTotal: parseFloat(art.dd_total) || 0,
            handlingRate: parseFloat(art.handling_rate) || 0,
            handlingTotal: parseFloat(art.handling_total) || 0,
            freight: parseFloat(art.freight) || 0,
            actWt: art.actual_weight?.toString() || '0',
            chargedWt: art.charged_weight?.toString() || '0',
      ...
      console.error(error);
      Alert.alert('Error', getErrorMessage(error, 'Failed to fetch GC details.'));
    } finally {
      setSearching(false);
    }
  };
  ...
  const handleSubmit = async () => {
    if (!formData.remarks || formData.remarks.trim() === '') {
      Alert.alert('Validation Error', 'Please add remarks explaining the reason for modification.');
      return;
    }
    ...
    try {
      ...
      const response = await api.put(`/waybills/${waybillId}`, payload);
      if (response.data.success) {
        Alert.alert('Success', 'GC Updated Successfully!');
      } else {
        Alert.alert('Error', response.data.message || 'Failed to update GC');
      }
    } catch (error) {
      Alert.alert('Error', getErrorMessage(error, 'Server Error updating GC'));
    } finally {
      setSaving(false);
    }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Modify GC</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        <View style={styles.accordionCard}>
          <TouchableOpacity style={styles.accordionHeader} onPress={() => toggleSection('search')}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={[styles.iconBox, { backgroundColor: '#e0e7ff' }]}>
                <Ionicons name="search" size={20} color="#4f46e5" />
              </View>
              <Text style={styles.accordionTitle}>Search GC Number</Text>
            </View>
            <Ionicons name={activeSection === 'search' ? 'chevron-up' : 'chevron-down'} size={24} color="#64748b" />
          </TouchableOpacity>
              ...
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 10 }}>
                  <TextInput 
                    ...
                    value={gcSearchNumber} 
                    onChangeText={setGcSearchNumber} 
                  />
                </View>
                <TouchableOpacity 
                  style={{ backgroundColor: '#4f46e5', paddingHorizontal: 20, justifyContent: 'center', borderRadius: 8 }}
                  onPress={searchGC}
                  disabled={searching}
                >
                  {searching ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontWeight: 'bold' }}>Search</Text>}
                </TouchableOpacity>
              </View>
            </View>
          )}
              ...
              {activeSection === 'route' && (
                <View style={styles.accordionBody}>
                  <View style={styles.inputGroup}>
                    ...
                    {showDatePicker && (
                      <DateTimePicker
                        value={new Date(formData.billDate || new Date().toISOString().split('T')[0])}
                        mode="date"
                        display="default"
                  ...
                  <CustomSelect 
                    ...
                    options={originBranches.map(b => ({id: b.id, label: b.branch_name}))}
        ...
        {waybillId && (
          <View style={styles.footer}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <Text style={{ fontSize: 16, color: '#64748b', fontWeight: '600' }}>Grand Total</Text>
              <Text style={{ fontSize: 24, fontWeight: '900', color: '#16a34a' }}>₹ {formData.grandTotal}</Text>
            </View>
            <TouchableOpacity 
              style={[styles.saveBtn, saving && { backgroundColor: '#94a3b8' }]}
            ...
            >
              {saving ? <ActivityIndicator color="#fff" /> : (
                <>
                  ...
                  <Text style={styles.saveBtnText}>Update GC</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#ffffff', borderTopWidth: 1, borderTopColor: '#e2e8f0', padding: 20, paddingBottom: Platform.OS === 'ios' ? 30 : 20, shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 10 },
  saveBtn: { backgroundColor: '#4f46e5', borderRadius: 12, paddingVertical: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
